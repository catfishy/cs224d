...
class RNN_Model():
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            ...
            curr_node_tensor = tf.gather(embedding,[word_id])
            ### END YOUR CODE
        else:
            node_tensors.update(self.add_model(node.left))
            node_tensors.update(self.add_model(node.right))
            ### YOUR CODE HERE
            lower_embedding = tf.concat(1,[node_tensors[node.left],node_tensors[node.right]])
            curr_node_tensor = tf.nn.relu(tf.matmul(lower_embedding,W1) + b1)
            ### END YOUR CODE
    ...
    def loss(self, logits, labels):
        ...
        with tf.variable_scope('Projection',reuse=True):
            U = tf.get_variable('U')
        ce_loss = tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(logits, labels))
        l2_loss = tf.nn.l2_loss(W1) + tf.nn.l2_loss(U)
        loss = ce_loss + self.config.l2*l2_loss
        # END YOUR CODE
        return loss
