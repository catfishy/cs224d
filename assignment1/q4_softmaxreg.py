...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    sentVector = np.mean(np.array([wordVectors[tokens[_]] for _ in sentence]),axis=0)
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    grad_softmax = prob - y
    grad_weights = np.dot(features.T,grad_softmax)/N
    grad_reg = regularization*weights
    grad = grad_reg + grad_weights
