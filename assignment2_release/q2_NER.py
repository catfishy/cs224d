...
class NERModel(LanguageModel):
  ...
  def add_model(self, window):
    ...
    h = tf.tanh(tf.matmul(window_drop,W) + b1, name='h')
    h_drop = tf.nn.dropout(h,self.dropout_placeholder)
    output_all = tf.matmul(h_drop,U) + b2
    output = tf.nn.dropout(output_all,self.dropout_placeholder)
    tf.add_to_collection('total_loss', U)
    tf.add_to_collection('total_loss', W)
    tf.add_to_collection('total_loss', output)
    ### END YOUR CODE
    return output
  ...
  def add_loss_op(self, y):
    ...
    W = total_loss[1]
    output = total_loss[2]
    ce_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(output,self.labels_placeholder))
    reg_loss = (self.config.l2/2.0) * (tf.reduce_mean(tf.square(U)) + tf.reduce_mean(tf.square(W)))
