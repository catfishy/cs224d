...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      inputs = [tf.squeeze(_,[1]) for _ in tf.split(1,self.config.num_steps,embed)]
      ### END YOUR CODE
      return inputs
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope('Projection'):
        ...
        b_2 = tf.get_variable('b_2',
                              (len(self.vocab),),
                              dtype=tf.float32)
        outputs = [tf.matmul(h_t,U)+b_2 for h_t in rnn_outputs]
    ### END YOUR CODE
    return outputs
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope('InputDropout'):
        inputs = [tf.nn.dropout(_,self.dropout_placeholder) for _ in inputs]

    with tf.variable_scope('RNN') as scope:
        self.initial_state = tf.zeros((self.config.batch_size, self.config.hidden_size))
        state = self.initial_state
        rnn_outputs = []
        for step, e in enumerate(inputs):
            if step > 0:
            ...
            state = tf.nn.sigmoid(tf.matmul(state,H) + tf.matmul(e,I) + b_1)
            rnn_outputs.append(state)

    with tf.variable_scope('RNNDropout'):
      self.final_state = rnn_outputs[-1]
      rnn_outputs = [tf.nn.dropout(_,self.dropout_placeholder) for _ in rnn_outputs]
